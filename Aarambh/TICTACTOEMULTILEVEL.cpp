        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (TIC[i][j] != 'X' && TIC[i][j] != 'O') {
                    char backup = TIC[i][j];
                    TIC[i][j] = AIsign;
                    best = max(best, minimax(TIC, depth + 1, false, playersign, AIsign));
                    TIC[i][j] = backup;
                }
            }
        }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (TIC[i][j] != 'X' && TIC[i][j] != 'O') {
                    char backup = TIC[i][j];
                    TIC[i][j] = playersign;
                    best = min(best, minimax(TIC, depth + 1, true, playersign, AIsign));
                    TIC[i][j] = backup;
                }
            }
        }
...
bool iswin(char TIC[3][3], int playersign, int AIsign) {
    ...
    for (int i = 0; i < 3; i++) {
        if (TIC[i][0] == TIC[i][1] && TIC[i][1] == TIC[i][2]) {
            if (TIC[i][0] == playersign) return true;
            if (TIC[i][0] == AIsign) return true;
        }
    }
    for (int i = 0; i < 3; i++) {
        if (TIC[0][i] == TIC[1][i] && TIC[1][i] == TIC[2][i]) {
            if (TIC[0][i] == playersign) return true;
...
//AIgame functions end here. 




bool iswin(char TIC[3][3], int &player1win, int &player2win, map<int, int> boardmap) {
...
int getRandomMove(char TIC[3][3]) {
 ...
 int random_index = rand() % available_moves.size();//RAND() GENERATE SOME RANDOM NO.
    ...
    //WHEN WE DIVIDE THE RANDOM NO. WITH THE SIZE OF THE VECTOR IT WILL GIVE THE INDEX OF THE RANDOM NO. IN THE VECTOR AS THR RANDOM INDEX MUST LIE IN THE RANGE OF 0 TO SIZE OF VECTOR 
...
bool iswin(char TIC[3][3], int &playerwin, int &computerwin, map<int, int> &boardmap,int playersign,int computersign) {
    // Check for winning conditions in rows
    for (int i = 0; i < 3; i++) {
        if (TIC[i][0] == TIC[i][1] && TIC[i][1] == TIC[i][2]) {
...
int main() {
    ...
    if(a==1){
        ...
        cout << "SYMBOL FOR COMPUTER: " << computersign << endl;
        ...
        while (count != -1) {  //THIS LOOP WILL RUN UNTIL THE GAME IS OVER
            while (playerwin != 1 && computerwin != 1) {
                // Player move
        ...
        cout<<endl<<"YOU ARE PLAYING WITH ANOTHER PLAYER:----:"<<endl;
          // Tic-Tac-Toe game setup
            ...
            while (count != -1) {
                while (player1win != 1 && player2win != 1) {
                    ...
                    // Player 1's move
                    if (!iswin(TIC, player1win, player2win, boardmap)) {
         ...
         if (player1move >= 1 && player1move <= 9 && boardmap[player1move] != 'X' && boardmap[player1move] != 'O') {
                            ...
                            cout << "INVALID MOVE PLAYER 1 🤬🤬" << endl;
                        }
                    }
                    // Player 2's move
                    if (!iswin(TIC, player1win, player2win, boardmap)) {
         ...
         if (player2move >= 1 && player2move <= 9 && boardmap[player2move] != 'X' && boardmap[player2move] != 'O') {
               ...
               cout << "INVALID MOVE PLAYER 2 🤬🤬" << endl;
                        }
                    }
                    // Check for a winner or draw
                    iswin(TIC, player1win, player2win, boardmap);
                    if (player1win == 1 && player2win == 1) {
                        cout << "DRAW! 😳😳" << endl;
            ...
            int playermove, count = 1;
             while (count != -1) {
                        while (playerwin != 1 && AIwin != 1) {
                            ...
                            // Player  move
                            if (!iswin(TIC, playerwin, AIwin, boardmap)) {
      ...
      cout <<endl<< "AI MOVE (AVAILABLE SPOTS ARE DISPLAYED ON SCREEN): -->  "<<moveAI<<endl;
                                ...
                                print(TIC);
                                if (iswin(TIC, playersign, AIsign)) {
                                    print(TIC);
                                    cout << "AI wins!" << endl;
                                    break;
                                }
                            ...
                            // Check for a winner or draw
                            iswin(TIC, playerwin, AIwin, boardmap);
                            if (playerwin == 1 && AIwin == 1) {
                                cout << "DRAW! 😳😳" << endl;
                            }
                            else if (playerwin == 1) {
                    ...
                    cout << "PLAYER 2 WON " << AIwins << " TIMES" << endl;
                    return 0;
            }
